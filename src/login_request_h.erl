...
init(Req0,Opts) ->
	...
	#{<<"user_id">> := User_id,<<"username">> := Username,<<"password">> := Password} = jsx:decode(Data),
	Result = erpc:cast(?SERVER,?LOGIC,request_login_api,[{binary_to_list(User_id),binary_to_list(Username),binary_to_list(Password)}]),
